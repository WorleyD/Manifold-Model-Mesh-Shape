...
def load(filename):
    ...
    elif part[-1].lower() == "tri":
        return load_tri(filename)
    else:
        raise RuntimeError('file format "'+part[-1]+'" not supported')

def load_tri(filename):
    triangles = str(filename)
    vertices = filename.split(".")[0] + ".vert"
    tf = open(triangles, "r")
    vf = open(vertices, "r")
    vertex = []
    face = []
    ...
    for line in vf:
        ...
        vertex.append([float(x) for x in line])

    for line in tf:
        line = line.strip().split()
        #Tri files I have are 1 indexed
        face.append([int(x)-1 for x in line])    

    output = mesh()
    output.vertex = np.array(vertex)
    output.face = np.array(face)
    return output
